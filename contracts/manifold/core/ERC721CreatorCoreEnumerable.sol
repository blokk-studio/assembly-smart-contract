 ...
 */
abstract contract ERC721CreatorCoreEnumerable is
    ERC721CreatorCore,
    IERC721CreatorCoreEnumerable
{
    ...
    mapping(address => mapping(address => uint256))
        private _extensionBalancesByOwner;
    mapping(address => mapping(address => mapping(uint256 => uint256)))
        private _extensionTokensByOwner;
    ...
    {
        return
            interfaceId == type(IERC721CreatorCoreEnumerable).interfaceId ||
     ...
     */
    function totalSupplyExtension(address extension)
        public
        view
        virtual
        override
     ...
     */
    function tokenByIndexExtension(address extension, uint256 index)
        external
        view
        virtual
        override
    ...
    {
        require(
            index < totalSupplyExtension(extension),
            "ERC721Creator: Index out of bounds"
        );
     ...
     */
    function balanceOfExtension(address extension, address owner)
        public
        view
        virtual
        override
     ...
     */
    function tokenOfOwnerByIndexExtension(
        address extension,
        address owner,
        uint256 index
    )
    ...
    {
        require(
            index < balanceOfExtension(extension, owner),
            "ERC721Creator: Index out of bounds"
        );
     ...
     */
    function tokenByIndexBase(uint256 index)
        external
        view
        virtual
        override
     ...
     */
    function balanceOfBase(address owner)
        public
        view
        virtual
        override
     ...
     */
    function tokenOfOwnerByIndexBase(address owner, uint256 index)
        external
        view
        virtual
        override
    ...
    function _removeTokenFromOwnerEnumeration(
        ...
        uint256 lastTokenIndexByOwner = balanceOfExtension(
            tokenExtension_,
            from
        ) - 1;
        ...
        if (tokenIndexByOwner != lastTokenIndexByOwner) {
            uint256 lastTokenIdByOwner = _extensionTokensByOwner[
                tokenExtension_
            ][from][lastTokenIndexByOwner];

            _extensionTokensByOwner[tokenExtension_][from][
                tokenIndexByOwner
            ] = lastTokenIdByOwner; // Move the last token to the slot of the to-delete token
            _extensionTokensIndexByOwner[
                lastTokenIdByOwner
            ] = tokenIndexByOwner; // Update the moved token's index
        }
        _extensionBalancesByOwner[tokenExtension_][from] -= 1;

        // This also deletes the contents at the last position of the array
        delete _extensionTokensIndexByOwner[tokenId];
        delete _extensionTokensByOwner[tokenExtension_][from][
            lastTokenIndexByOwner
        ];
    }
    ...
    function _postMintBase(address to, uint256 tokenId)
        internal
        virtual
        override
    {
    ...
    function _postMintExtension(address to, uint256 tokenId)
        internal
        virtual
        override
    {
    ...
    function _postBurn(address owner, uint256 tokenId)
        internal
        virtual
        override(ERC721CreatorCore)
    {
        ...
        if (tokenIndex != lastTokenIndex) {
            uint256 lastTokenId = _extensionTokens[tokenExtension_][
                lastTokenIndex
            ];
