  ...
  describe("buy lot", function () {
    ...
    it("success erc721 and check royalties", async function () {
      ...
      expect(await artist1.getBalance()).to.be.equal(
        balances[0].add(lot.price.mul(50).div(100))
      );
      expect(await artist2.getBalance()).to.be.equal(
        balances[1].add(lot.price.mul(20).div(100))
      );
      expect(await buyer.getBalance()).to.be.equal(
        balances[2].sub(txPrice.add(price))
      );
      expect(await recipient.getBalance()).to.be.equal(
        balances[3].add(lot.price.mul(30).div(100))
      );
    });
