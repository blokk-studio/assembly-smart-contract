...
describe("AssemblyCurated_v3", function () {
  ...
  it("permissions", async function () {
    await expect(marketplace.connect(minter).updateRecipient(recipient.address)).to.be.revertedWith("Ownable: caller is not the owner");


    expect(
      await marketplace.isMinter(minter.address)
    ).to.be.equal(true);
    await expect(
      marketplace.addMinter(minter.address)
    ...
    await expect(
      marketplace.removeMinter(minter.address)
    ).to.be.revertedWith("AlreadySet()");
    await expect(
      marketplace.addMinter(
        "0x0000000000000000000000000000000000000000"
      )
    ).to.be.revertedWith("ZeroAddress()");
  ...
  describe('lazy minting', function () {
    ...
    beforeEach(async () => {
      lazyMinter = new LazyMinterV3(marketplace, minter);
    ...
    it('reject when voucher with invalid data', async function () {
      const voucher = await lazyMinter.createVoucher(erc1155.address, 0, 100, false, 0, '/test2/', 80, 20, [], []);
    ...
    it('reject with invalid length', async function () {
      const voucher2 = await lazyMinter.createVoucher(erc1155.address, 0, 100, true, 50, '/test2/', 80, 20, [minter.address], []);
      await expect(marketplace.connect(buyer).buyWithMint(buyer.address, voucher2, { value: BigNumber.from(voucher2.price).mul(voucher2.amount).mul(2) })).to.be.revertedWith('WrongArrayLength()');

    })

    it('reject when voucher with invalid signer', async function () {
      const invalidLazyMinter = new LazyMinterV3(marketplace, owner);
      const voucher = await invalidLazyMinter.createVoucher(erc1155.address, 0, 100, true, 1, '/test2/', 80, 20, [], []);
      await expect(marketplace.connect(buyer).buyWithMint(buyer.address, voucher)).to.be.revertedWith('InvalidSignature()');
    })

    it("reject when not enough value", async function () {
      const voucher = await lazyMinter.createVoucher(erc1155.address, 0, 1000000, true, 1, '/test2/', 80, 20, [], []);
    ...
    it('reject when invalid amount', async function () {
      const voucher = await lazyMinter.createVoucher(erc1155.address, 0, 1000000, true, 0, '/test2/', 80, 20, [], []);
    ...
    it('reject when token is not available for lazy minting', async function () {
      ...
      const voucher = await lazyMinter.createVoucher(erc1155.address, 0, 1000000, true, 50, '/test2/', 80, 20, [], []);
    ...
    it('reject when try mint not existing 1155', async function () {
      const voucher = await lazyMinter.createVoucher(erc1155.address, 10, 1000000, true, 50, '/test2/', 80, 20, [], []);
    ...
    it('reject when voucher fees not equal 100', async function () {
      let voucher = await lazyMinter.createVoucher(erc1155.address, 10, 1000000, true, 50, '/test2/', 0, 0, [minter.address], [0]);
      await expect(marketplace.connect(buyer).buyWithMint(buyer.address, voucher, { value: BigNumber.from(voucher.price).mul(voucher.amount) })).to.be.revertedWith('InvalidVoucherFees()');

      voucher = await lazyMinter.createVoucher(erc1155.address, 10, 1000000, true, 50, '/test2/', 80, 0, [minter.address], [0]);
      await expect(marketplace.connect(buyer).buyWithMint(buyer.address, voucher, { value: BigNumber.from(voucher.price).mul(voucher.amount) })).to.be.revertedWith('InvalidVoucherFees()');

      voucher = await lazyMinter.createVoucher(erc1155.address, 10, 1000000, true, 50, '/test2/', 80, 0, [minter.address], [5]);
      await expect(marketplace.connect(buyer).buyWithMint(buyer.address, voucher, { value: BigNumber.from(voucher.price).mul(voucher.amount) })).to.be.revertedWith('InvalidVoucherFees()');

      voucher = await lazyMinter.createVoucher(erc1155.address, 10, 1000000, true, 50, '/test2/', 80, 30, [minter.address], [0]);
      await expect(marketplace.connect(buyer).buyWithMint(buyer.address, voucher, { value: BigNumber.from(voucher.price).mul(voucher.amount) })).to.be.revertedWith('InvalidVoucherFees()');

      voucher = await lazyMinter.createVoucher(erc1155.address, 10, 1000000, true, 50, '/test2/', 0, 0, [minter.address], [90]);
      await expect(marketplace.connect(buyer).buyWithMint(buyer.address, voucher, { value: BigNumber.from(voucher.price).mul(voucher.amount) })).to.be.revertedWith('InvalidVoucherFees()');
    ...
    it('success 721 with equal value', async function () {
      ...
      const voucher = await lazyMinter.createVoucher(erc721.address, 0, 100, false, 0, '/test2/', 80, 20, [], []);
    ...
    it('success new 1155 with dusts', async function () {
      ...
      const voucher = await lazyMinter.createVoucher(erc1155.address, 0, 100, true, 50, '/test2/', 80, 20, [], []);
    ...
    it("success existing 1155", async function () {
      ...
      const voucher2 = await lazyMinter.createVoucher(erc1155.address, 0, 100, true, 100, '/test2/', 80, 20, [], []);
      ...
      const balanceEth = await buyer.getBalance();
      const voucher = await lazyMinter.createVoucher(erc1155.address, 1, 100, true, 50, '/test2/', 80, 20, [], []);
    ...
    it("pausable", async function () {
      ...
      await expect(marketplace.connect(artist1).pause()).to.be.revertedWith("Ownable: caller is not the owner");
      ...
      const voucher2 = await lazyMinter.createVoucher(erc1155.address, 0, 100, true, 100, '/test2/', 80, 20, [], []);
      await expect(marketplace.connect(artist1).buyWithMint(artist1.address, voucher2, { value: BigNumber.from(voucher2.price).mul(voucher2.amount) })).to.be.revertedWith("Pausable: paused");
    ...
    it("success check fees", async function () {
      ...
      const voucher2 = await lazyMinter.createVoucher(erc1155.address, 0, 100, true, 100, '/test2/', 50, 20, [minter.address, artist1.address], [25, 5]);
      await marketplace.connect(buyer).buyWithMint(buyer.address, voucher2, { value: BigNumber.from(voucher2.price).mul(voucher2.amount) });
      const totalPtice = BigNumber.from(100 * 100);
      expect(await recipient.getBalance()).to.be.equal(recieptBalance.add(totalPtice.mul(50).div(100)));
      expect(await owner.getBalance()).to.be.equal(ownerBalance.add(totalPtice.mul(20).div(100)));
      expect(await minter.getBalance()).to.be.equal(minterBalance.add(totalPtice.mul(25).div(100)));
      expect(await artist1.getBalance()).to.be.equal(artist1Balance.add(totalPtice.mul(5).div(100)));
